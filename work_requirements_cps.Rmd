...
## Plot data
```{r}
medicaid_year <- svytable(~medicaid + YEAR + intervention2, sample2) %>%
  as.data.frame() %>%
  group_by(YEAR, intervention2) %>%
  summarize(
    total = sum(Freq),
...
medicaid_year_plot <- ggplot(medicaid_year, aes(x = YEAR, y = percentage, color = intervention2, group = intervention2)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Has Medicaid",
  ...
  scale_color_discrete(labels = c("Non-expansion states", "Georgia")) +
  theme_minimal() +
...
# Uninsured
uninsured_year <- svytable(~uninsured + YEAR + intervention2, sample2) %>%
  as.data.frame() %>%
  group_by(YEAR, intervention2) %>%
  summarize(
    total = sum(Freq),
...
uninsured_year_plot <- ggplot(uninsured_year, aes(x = YEAR, y = percentage, color = intervention2, group = intervention2)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Uninsured",
...
# Employed
employed_year <- svytable(~employed + YEAR + intervention2, sample2) %>%
  as.data.frame() %>%
  group_by(YEAR, intervention2) %>%
  summarize(
    total = sum(Freq),
...
employed_year_plot <- ggplot(employed_year, aes(x = YEAR, y = percentage, color = intervention2, group = intervention2)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Employed",
