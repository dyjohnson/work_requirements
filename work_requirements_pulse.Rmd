...
data <- data %>% 
  mutate(
    post = case_when( # Blank out April 1, 2023 - July 31, 2023
      date < ymd("2023-04-01") ~ 0,
      date > ymd("2023-07-31") ~ 1,
      TRUE ~ NA
    ),
...
sample <- survey %>%
  subset(
    # post %in% c(FALSE, TRUE) & # Exclude blanked period
...
```{r}
# Medicaid
medicaid_quarter <- svyby(~medicaid, ~quarter_factor + group_factor, sample,
...
medicaid_quarter_plot <- ggplot(medicaid_quarter, aes(x = quarter_factor, y = percentage, color = group_factor, group = group_factor)) +
  geom_line() +
  geom_point() +
...
ggsave("medicaid_quarter_plot_3.jpg")

# Employer-sponsored insurance
esi_quarter <- svyby(~esi, ~quarter_factor + group_factor, sample,
                       svymean, vartype = "se", na.rm = TRUE) %>%
  as.data.frame() %>%
  rename(
    percentage = esi,
...
esi_quarter_plot <- ggplot(esi_quarter, aes(x = quarter_factor, y = percentage, color = group_factor, group = group_factor)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_vline(xintercept = policy_change_x, linetype = "dashed", color = "black") +  # Policy change in July 2023
...
esi_quarter_plot
ggsave("esi_quarter_plot_3.jpg")

# Uninsured
uninsured_quarter <- svyby(~uninsured, ~quarter_factor + group_factor, sample,
                       svymean, vartype = "se", na.rm = TRUE) %>%
  as.data.frame() %>%
  rename(
...
uninsured_quarter_plot <- ggplot(uninsured_quarter, aes(x = quarter_factor, y = percentage, color = group_factor, group = group_factor)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_vline(xintercept = policy_change_x, linetype = "dashed", color = "black") +  # Policy change in July 2023
...
ggsave("uninsured_quarter_plot_3.jpg")

# Employed
employed_quarter <- svyby(~employed, ~quarter_factor + group_factor, sample,
                       svymean, vartype = "se", na.rm = TRUE) %>%
  as.data.frame() %>%
  rename(
    percentage = employed,
...
employed_quarter_plot <- ggplot(employed_quarter, aes(x = quarter_factor, y = percentage, color = group_factor, group = group_factor)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_vline(xintercept = policy_change_x, linetype = "dashed", color = "black") +  # Policy change in July 2023
...
ggsave("employed_quarter_plot_3.jpg")
```
...
gen_did_tbl <- function(outcome_var, unadj_model, adj_model, survey_design, z_value = 1.96) {
  ...
  cat("\nUnadjusted Model:\n")
  cat(paste0("Difference-in-Difference Estimate: ", formatC(unadj_diff_in_diff, format = "f", digits = 1), " (",
      formatC(unadj_ci[1], format = "f", digits = 1), ", ",
      formatC(unadj_ci[2], format = "f", digits = 1), ")\n"))
  cat(paste0("p-value: ", formatC(unadj_p_value, format = "f", digits = 1), "\n"))

  # Print the results for the adjusted model
  cat(paste0("\nAdjusted Model:\n"))
  cat(paste0("Difference-in-Difference Estimate: ", formatC(adj_diff_in_diff, format = "f", digits = 1), " (",
      formatC(adj_ci[1], format = "f", digits = 1), ", ",
      formatC(adj_ci[2], format = "f", digits = 1), ")\n"))
  cat(paste0("p-value: ", formatC(adj_p_value, format = "f", digits = 1), "\n"))
}
