...
data <- data %>% 
  mutate(
    ...
    intervention = case_when(
      ...
      state %in% c("Mississippi", "South Carolina", "Tennessee", "Alabama", "Florida", "Texas") ~ 0,
      TRUE ~ NA_real_
    )
  )
...
eligible_post <- eligible %>% subset(post == 1)
```

# Plot data - dates
```{r}
# Medicaid coverage
medicaid_date <- svytable(~medicaid + date + intervention, eligible) %>%
  as.data.frame() %>%
...
# ESI
esi_date <- svytable(~esi + date + intervention, eligible) %>%
  as.data.frame() %>%
  group_by(date, intervention) %>%
  summarize(
    total = sum(Freq),
...
# Uninsured
uninsured_date <- svytable(~uninsured + date + intervention, eligible) %>%
  as.data.frame() %>%
  group_by(date, intervention) %>%
  summarize(
    total = sum(Freq),
...
# Employed
employed_date <- svytable(~employed + date + intervention, eligible) %>%
  as.data.frame() %>%
  group_by(date, intervention) %>%
  summarize(
    total = sum(Freq),
...
```{r}
# Medicaid coverage
medicaid_quarter <- svytable(~medicaid + quarter + state, eligible) %>%
  as.data.frame() %>%
...
# ESI
esi_quarter <- svytable(~esi + quarter + state, eligible) %>%
  as.data.frame() %>%
  group_by(quarter, state) %>%
  summarize(
    total = sum(Freq),
...
# Uninsured
uninsured_quarter <- svytable(~uninsured + quarter + state, eligible) %>%
  as.data.frame() %>%
  group_by(quarter, state) %>%
  summarize(
    total = sum(Freq),
...
# Employed
employed_quarter <- svytable(~employed + quarter + state, eligible) %>%
  as.data.frame() %>%
  group_by(quarter, state) %>%
  summarize(
    total = sum(Freq),
...
```{r}
# Medicaid
medicaid_pttest <- svyglm(medicaid ~ intervention * date, design = eligible_pre)
summary(medicaid_pttest)

# Employer-sponsored insurance
esi_pttest <- svyglm(esi ~ intervention * date, design = eligible_pre)
summary(esi_pttest)

# Uninsured
uninsured_pttest <- svyglm(uninsured ~ intervention * date, design = eligible_pre)
summary(uninsured_pttest)

# Employed
employed_pttest <- svyglm(employed ~ intervention * date, design = eligible_pre)
...
# Uninsured
uninsured_adj <- svyglm(uninsured ~ intervention * post + age_cat + sex_cat, design = eligible)
summary(uninsured_adj)

# Employed
employed_adj <- svyglm(employed ~ intervention * post + age_cat + sex_cat, design = eligible)
...
# Playground
```{r}
eligible_medicaid <- subset(eligible, !is.na(medicaid))

medicaid_adj2 <- svyglm(medicaid ~ intervention * post + age_cat + sex_cat + state_factor, design = eligible)
...
deff_result <- svymean(~medicaid, eligible_medicaid, deff = TRUE)
deff_result
