...
# Factor state and date
# data <- data %>%
#   mutate(
#     state_factor = case_when(
#       state %in% c("Georgia", "Mississippi", "South Carolina", "Tennessee", "Alabama", "Florida", "Texas") ~ as.factor(state),
#       TRUE ~ NA
#     )
#   )
...
medicaid_quarter_plot <- ggplot(medicaid_quarter, aes(x = quarter, y = percentage, color = intervention, group = intervention)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +s
  labs(
    x = "Quarter",
    y = "Medicaid, %"
  ) +
...
# Plot data - state quarters
```{r}
# Medicaid coverage
medicaid_quarter <- svytable(~medicaid + quarter + state, sample_pre) %>%
  as.data.frame() %>%
...
medicaid_quarter_plot <- ggplot(medicaid_quarter, aes(x = quarter, y = percentage, color = state, group = state)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Medicaid",
...
medicaid_quarter_plot
ggsave("medicaid_quarter_plot_states.jpg")

# ESI
esi_quarter <- svytable(~esi + quarter + state, sample_pre) %>%
  as.data.frame() %>%
  group_by(quarter, state) %>%
  summarize(
    total = sum(Freq),
...
esi_quarter_plot <- ggplot(esi_quarter, aes(x = quarter, y = percentage, color = state, group = state)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Employer-sponsored insurance",
...
esi_quarter_plot
ggsave("esi_quarter_plot_states.jpg")

# Uninsured
uninsured_quarter <- svytable(~uninsured + quarter + state, sample_pre) %>%
  as.data.frame() %>%
  group_by(quarter, state) %>%
  summarize(
    total = sum(Freq),
...
uninsured_quarter_plot <- ggplot(uninsured_quarter, aes(x = quarter, y = percentage, color = state, group = state)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Uninsured",
...
uninsured_quarter_plot
ggsave("uninsured_quarter_plot_states.jpg")

# Employed
employed_quarter <- svytable(~employed + quarter + state, sample_pre) %>%
  as.data.frame() %>%
  group_by(quarter, state) %>%
  summarize(
    total = sum(Freq),
...
employed_quarter_plot <- ggplot(employed_quarter, aes(x = quarter, y = percentage, color = state, group = state)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Employed",
...
employed_quarter_plot
ggsave("employed_quarter_plot_states.jpg")
```
...
# Playground
```{r}
sample_filtered <- update(sample, state_factor2 = as.factor(state))

is.factor(sample_filtered$variables$state_factor2)

sample_filtered <- update(sample, date2 = ifelse(!is.na(post), date, NA))

sample_filtered <- update(sample_filtered, date_factor2 = as.factor(date2))

is.factor(sample_filtered$variables$date_factor2)

sample_filtered <- subset(sample_filtered, !is.na(date_factor2))

sample_filtered <- update(sample, state2 = ifelse(!is.na(intervention), state, NA))

sample_filtered <- subset(sample_filtered, !is.na(state) & !is.na(post))

medicaid_adj2 <- svyglm(medicaid ~ intervention * post + as.character(state), design = sample_filtered)
summary(medicaid_adj2)



clustered_se <- vcovCL(medicaid_adj2, cluster = ~state_factor, type = "HC0")
coeftest(medicaid_adj, vcov = clustered_se)

deff_result <- svymean(~medicaid, sample_medicaid, deff = TRUE)
deff_result

# Extract model matrix from `svyglm`
model_matrix <- model.frame(medicaid_adj)

# Apply clustered standard errors at the state level
clustered_se <- vcovCL(medicaid_adj, cluster = model_matrix$state_factor, type = "HC0")

# Compute clustered standard errors at the state level
#clustered_se <- vcovCL(medicaid_adj, cluster = ~state_factor, type = "HC0")

# Get coefficient estimates with clustered standard errors
coeftest(medicaid_adj, vcov = clustered_se)
...
# Extract model matrix from `svyglm`
model_matrix <- model.frame(employed_pttest)

# Apply clustered standard errors at the state level
clustered_se <- vcovCL(employed_pttest, cluster = model_matrix$state_factor, type = "HC0")

# Get coefficient estimates with clustered standard errors
coeftest(employed_pttest, vcov = clustered_se)
```