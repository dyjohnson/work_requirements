...
# table(merge_2223$post, merge_2223$IYEAR, useNA = "ifany")
# table(merge_2223$intervention, merge_2223$`_STATE`, useNA = "ifany")
# table(merge_2223$quarter, merge_2223$IMONTH, useNA = "ifany")
# table(merge_2223$half, merge_2223$IMONTH, useNA = "ifany")
```
...
svytable(~disability + medicaid, survey_2223, exclude=NULL, na.action=na.pass)
```
...
```{r}
# Medicaid
medicaid_unadj <- svyglm(medicaid ~ intervention * prepost, design = eligible)
summary(medicaid_unadj)

# Uninsured
uninsured_unadj <- svyglm(uninsured ~ intervention * prepost, design = eligible)
summary(uninsured_unadj)

# Qualifying activities
qualifying_unadj <- svyglm(qualifying ~ intervention * prepost, design = eligible)
summary(qualifying_unadj)

# Employed
employed_unadj <- svyglm(employed ~ intervention * prepost, design = eligible)
summary(employed_unadj)

# Student
student_unadj <- svyglm(student ~ intervention * prepost, design = eligible)
summary(student_unadj)

# Employer-sponsored insurance
esi_unadj <- svyglm(esi ~ intervention * prepost, design = eligible)
summary(esi_unadj)

# Access to PCP
pcp_unadj <- svyglm(pcp ~ intervention * prepost, design = eligible)
summary(pcp_unadj)

# Cost-related delays in care
not_afford_unadj <- svyglm(not_afford ~ intervention * prepost, design = eligible)
...
not_afford_quarter_plot <- ggplot(not_afford_quarter, aes(x = quarter, y = percentage, color = intervention, group = intervention)) +
  ...
  labs(
    title = "Could not afford to see doctor in past 12 months",
    x = "quarter",
    y = "Could not afford to see doctor in past 12 months",
    color = "intervention"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Tabulate weighted respondent characteristics in pre periods
# group_tab_w <- svytable(~ intervention, design = eligible_pre) %>% as.data.frame()
# group_tab_w
...
education_tbl_pre <- gen_table_chisq("education", "intervention", eligible_pre)
...
intervention_tab_w <- svytable(~ intervention, design = eligible_post) %>% as.data.frame()
intervention_tab_w
...
education_tbl_post <- gen_table_chisq("education", "intervention", eligible_post)
...
# Run on outcomes
gen_did_tbl("medicaid", eligible)
gen_did_tbl("esi", eligible)
gen_did_tbl("uninsured", eligible)
gen_did_tbl("qualifying", eligible)
gen_did_tbl("employed", eligible)
gen_did_tbl("student", eligible)
gen_did_tbl("pcp", eligible)
gen_did_tbl("not_afford", eligible)
```